...
- Frontend implementation track: In progress (phase 4 test baseline)
  - Completed:
    ...
    - single-call login session hydration (removed extra `/auth/me` call during sign-in)
  - Remaining:
...
- Progress update (2026-03-18):
  ...
  - Added deploy gate in GitHub Pages workflow:
    - backend unit + e2e smoke must pass before Pages build/deploy runs
